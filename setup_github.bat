...
set /p REPO_NAME="저장소명 입력 (기본값: portfolio-signal-mailer): "

if "%REPO_NAME%"=="" set REPO_NAME=portfolio-signal-mailer

echo.
echo [단계 1/3] Git 저장소 설정...

REM git init은 이미 되었으므로 remote 추가만
...
(
    ...
    echo 
    echo # 민감한 파일
    echo signal_mailer/config.yaml
    echo signal_mailer/*.log
    echo .env
    echo 
    echo # OS
    echo .DS_Store
    echo Thumbs.db
    echo 
    echo # crash_detection_system은 제외
    echo crash_detection_system/
) > .gitignore
