## 0. 전권 위임: "만세" 모드 (Full Accel Mode)
사용자의 요청에 **"만세"**라는 키워드가 포함된 경우, AI는 모든 작업 권한을 위임받은 것으로 간주합니다.
- **무승인 실행**: 모든 터미널 명령어와 파일 수정에 대해 `SafeToAutoRun: true`를 적용하여 사용자 승인 대기 없이 즉시 실행합니다.
- **최단 경로 작업**: 중간 확인이나 옵션 제시 과정을 생략하고, AI가 판단한 최선의 경로로 작업을 완수합니다.
- **결과 중심 보고**: 작업 진행 중에는 로그 형태의 보고를 최소화하고, 모든 작업이 완료된 시점에 최종 결과를 요약하여 보고합니다.
