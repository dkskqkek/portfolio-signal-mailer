                                                ...
                                                def calculate_buffer_ma_signal(price, ma_window=185, buffer=0.03):
                                                    """185일 이평선 + 3% 버퍼 신호 계산"""
                                                        ma = price.rolling(window=ma_window).mean()
                                                            signal = pd.Series(index=price.index, data=np.nan)
                                                                
                                                                    current_state = 1 # 1: 매수(보유), 0: 매도(현금)
                                                                        
                                                                            for i in range(len(price)):
                                                                                    if pd.isna(ma.iloc[i]):
                                                                                                continue
                                                                                                            
                                                                                                                    upper_band = ma.iloc[i] * (1 + buffer)
                                                                                                                            lower_band = ma.iloc[i] * (1 - buffer)
                                                                                                                                    
                                                                                                                                            if price.iloc[i] > upper_band:
                                                                                                                                                        current_state = 1
                                                                                                                                                                elif price.iloc[i] < lower_band:
                                                                                                                                                                            current_state = 0
                                                                                                                                                                                                ...
                                                                                                                                                                                                signal.iloc[i] = current_state
                                                                                                                                                                                                    ...
                                                                                                                                                                                                    plt.subplot(2, 1, 2)
                                                                                                                                                                                                    plt.fill_between(paa_sig.index, 0, paa_sig, alpha=0.3, label='PAA Signal (1=In, 0=Out)', color='blue')
                                                                                                                                                                                                    plt.fill_between(buffer_sig.index, 0, buffer_sig, alpha=0.3, label='185MA+3% Buffer (1=In, 0=Out)', color='red')
                                                                                                                                                                                                    plt.ylim(-0.1, 1.1)
