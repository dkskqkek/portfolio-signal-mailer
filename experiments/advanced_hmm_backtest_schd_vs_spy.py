# -*- coding: utf-8 -*-
"""
고급 HMM/Kalman 시그널 백테스트: SCHD vs SPY
- 기본 시그널 대신 crash_detection_system의 고도화된 레짐 감지 사용
- Bull/Correction/Crisis 3단계 레짐 기반 포트폴리오 전환
"""
...
def get_advanced_signals(df):
    """고급 HMM/Kalman 시그널 생성"""
    print("\n🧠 고급 HMM/Kalman 엔진 실행 중...")
    
    try:
        # CrashDetectionPipeline 실행
        ...
        if results['status'] != 'SUCCESS':
            print(f"  ✗ HMM 파이프라인 실패: {results.get('error', 'Unknown')}")
            return None
        
        # 시그널 추출
        signals_df = pipeline.signals
        
        # 날짜 인덱스 정렬
        signals_df.index = pd.to_datetime(signals_df.index).tz_localize(None)
        
        # df와 병합
        df = df.join(signals_df[['signal', 'signal_reason']], how='left')
        
        # 신호 매핑: -2(Strong Sell) ~ 2(Strong Buy)
        # Danger: signal <= 0
        # Normal: signal > 0
        df['is_danger'] = (df['signal'] <= 0).astype(int)
        df['is_danger'] = df['is_danger'].fillna(0).astype(int)
        
        print(f"  ✓ HMM 시그널 생성 완료")
        print(f"  - 위험 신호: {df['is_danger'].sum()}일 ({df['is_danger'].sum()/len(df)*100:.1f}%)")
        print(f"  - 정상 신호: {(1-df['is_danger']).sum()}일 ({(1-df['is_danger']).sum()/len(df)*100:.1f}%)")
...
def main():
    print("=" * 60)
    print("🧠 고급 HMM 시그널 백테스트: SCHD vs SPY")
    print("=" * 60)
    ...
    # 고급 시그널 생성
    df = get_advanced_signals(df)
    
    if df is None or 'is_danger' not in df.columns:
        print("\n❌ HMM 시그널 생성 실패. 기본 시그널로 대체합니다.")
        # 기본 시그널로 폴백
        spy = df['SPY']
        log_ret = np.log(spy / spy.shift(1))
        roll_mean = log_ret.rolling(20).mean()
    ...
    # 포트폴리오 백테스트
    portfolio_a = backtest_portfolio(df, 'SCHD', 'Portfolio A (SCHD + HMM)')
    portfolio_b = backtest_portfolio(df, 'SPY', 'Portfolio B (SPY + HMM)')
    ...
    plt.subplot(2, 1, 1)
    plt.plot(portfolio_a.index, portfolio_a, label='Portfolio A (SCHD + HMM)', linewidth=2)
    plt.plot(portfolio_b.index, portfolio_b, label='Portfolio B (SPY + HMM)', linewidth=2, alpha=0.8)
    plt.title('고급 HMM 시그널 백테스트: SCHD vs SPY', fontsize=14, fontweight='bold')
    ...
    plt.title('HMM Market Regime (QQQ ↔ JEPI Switching)', fontsize=12)
    plt.ylabel('Signal')
    plt.xlabel('Date')
    plt.ylim(-0.1, 1.1)
    plt.legend(loc='upper left')
    ...
    plt.tight_layout()
    plt.savefig('advanced_hmm_comparison_schd_vs_spy.png', dpi=150)
    print(f"\n📈 차트 저장: advanced_hmm_comparison_schd_vs_spy.png")
    
    # 리포트 저장
    with open('advanced_hmm_comparison_report.txt', 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("고급 HMM 시그널 백테스트: SCHD vs SPY\n")
        f.write("=" * 60 + "\n\n")
        f.write(f"기간: {df.index[0].date()} ~ {df.index[-1].date()}\n")
        f.write(f"초기 자본: ${INITIAL_CAPITAL:,}\n")
        f.write(f"시그널: 고급 HMM/Kalman 레짐 감지\n\n")
        f.write("포트폴리오 구성:\n")
    ...
    print(f"📄 리포트 저장: advanced_hmm_comparison_report.txt\n")
