...
def evaluate_portfolio(w_spy, w_qqq, w_gld, w_ks):
    # w_qqq는 Tactical 비중 (Switching 대상)
    # 나머지는 Fixed 비중
    ...
    for i in range(1, len(data)):
        ...
        # Tactical Portion (QQQ or Ensemble)
...
results = []
# 가중치 조합 생성 (5% 단위)
for w_qqq in range(10, 85, 5): # QQQ Tactical 비중 10%~80%
    for w_spy in range(0, 85, 5):
        for w_gld in range(5, 25, 5): # GLD 5%~20%
            for w_ks in range(5, 35, 5): # KOSPI 5%~30%
...
df.to_csv("portfolio_optimization_results.csv")
print("\nOptimization results saved to portfolio_optimization_results.csv")
